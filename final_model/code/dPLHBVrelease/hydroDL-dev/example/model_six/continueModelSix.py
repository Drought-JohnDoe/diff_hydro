...
def parse_args():
    parser = argparse.ArgumentParser(description='Resume Model Six training')
...
def main():
    ...
    if args.use_all_basins:
        ...
        train_ls = read_subset_file(subset_file)[:args.subset_limit]
    ...
    out = Path(root_out) / exp_name / exp_disp / exp_info

    with open(out / 'statDict.json', 'r') as fp:
    ...
    model = loadModel(str(out), epoch=args.start_epoch)
    if torch.cuda.is_available():
        model = model.cuda()
    optim = torch.optim.Adadelta(model.parameters())
    model.zero_grad()

    run_file = out / 'run.csv'
    resume_file = out / f'run_resume_from_{args.start_epoch}_to_{args.end_epoch}.csv'
    with open(run_file, 'a') as rf, open(resume_file, 'w') as rrf:
        for i_epoch in range(args.start_epoch + 1, args.end_epoch + 1):
            loss_ep = 0.0
            t0 = time.time()
            for i_iter in range(args.max_iter_ep):
                i_grid, i_t = train.randomIndex(len(train_ls), x_train.shape[1], [args.batch_size, args.rho], bufftime=bufftime)
                x_train_batch = train.selectSubset(x_train, i_grid, i_t, args.rho, bufftime=bufftime)
                y_train_batch = train.selectSubset(y_train, i_grid, i_t, args.rho)
                z_train_batch = train.selectSubset(z_train, i_grid, i_t, args.rho, c=attrs)
                ...
                loss_ep += loss.item()
                if i_iter % 100 == 0:
                    print(f'Iter {i_iter} of {args.max_iter_ep}: Loss {loss.item():.3f}', flush=True)
            loss_ep /= args.max_iter_ep
            log_str = f'Epoch {i_epoch} Loss {loss_ep:.3f} time {time.time() - t0:.2f}'
            print(log_str, flush=True)
            rf.write(log_str + '\n')
            rf.flush()
            rrf.write(log_str + '\n')
            rrf.flush()
            torch.save(model, out / f'model_Ep{i_epoch}.pt')

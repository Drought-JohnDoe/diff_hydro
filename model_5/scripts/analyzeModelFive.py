...
def parse_args():
    parser = argparse.ArgumentParser(description='Analyze Model Five diagnostics')
...
def main():
    ...
    exp_name = 'CAMELSMODELFIVE'
    subset_tag = 'AllBasins' if args.use_all_basins else 'Subset' + str(len(basin_ids))
    exp_disp = 'DynamicSimHydModelFive/' + subset_tag + '/' + args.forcing + '/' + str(args.seed)
    exp_info = (
    ...
    with torch.no_grad():
        ...
        theta_w = torch.sum(theta * wts.unsqueeze(1), dim=2).detach().cpu().numpy()
        wts_np = wts.detach().cpu().numpy()
        rout_np = rout0.detach().cpu().numpy()

    param_names = ['INSC', 'COEF', 'SQ', 'SMSC', 'SUB', 'CRAK', 'K', 'LG', 'TT', 'CFMAX', 'CFR', 'CWH', 'SG_CRIT']
    for i, nm in enumerate(param_names):
        meta[nm + '_wavg'] = theta_w[:, i]
    meta['route_a_static'] = rout_np[:, 0]
    meta['route_b_static'] = rout_np[:, 1]
    for i in range(wts_np.shape[1]):
        meta['weight_c' + str(i + 1)] = wts_np[:, i]

    np.savez_compressed(
        os.path.join(out_dir, 'model_five_diagnostics_ep' + str(args.epoch) + '.npz'),
    ...
    meta.to_csv(os.path.join(out_dir, 'per_basin_model_five_metrics.csv'), index=False)
    ...
    for nm in ['INSC_wavg', 'COEF_wavg', 'SQ_wavg', 'SMSC_wavg', 'SUB_wavg', 'CRAK_wavg',
        ...
        plot_map(meta, nm, 'Model Five static ' + nm, os.path.join(map_param_dir, nm + '.png'), cmap='viridis')
    ...
    for nm in ['mean_groundwater_storage', 'sum_groundwater_loss', 'sum_baseflow',
        ...
        plot_map(meta, nm, 'Model Five flux/metric ' + nm, os.path.join(map_flux_dir, nm + '.png'), cmap=cmap)
